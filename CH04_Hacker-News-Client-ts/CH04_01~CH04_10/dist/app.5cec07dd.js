...
function (_super) {
  ...
  NewsFeedView.prototype.render = function () {
    ...
    window.store.limit = Math.ceil(this.feeds.length / window.store.offset);

    for (var i = (window.store.currentPage - 1) * window.store.offset; i < window.store.currentPage * window.store.offset; i++) {
      var _a = this.feeds[i],
          id = _a.id,
          title = _a.title,
    ...
    this.setTemplateData("next_page", String(window.store.currentPage < window.store.limit ? window.store.currentPage + 1 : window.store.limit));
    this.updateView();
  };
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "59992" + '/');
